...
class App
  ...
  def create_teacher(age, specializaiton, name)
    @people << Teacher.new(age, specializaiton, name)
  end
  ...
  def list_rental(person)
    ...
    person.rentals.each {|rental| puts "Title: #{rental.book.title} ,  Author: #{rental.book.author} ,  Date: #{rental.date}"}
  end
end